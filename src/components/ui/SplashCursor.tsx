...
export default function SplashCursor({
  ...
  useEffect(() => {
    if (!mounted || !hasMeasured || isMobile) return;
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement) {
      ...
      let glCtx = canvasEl.getContext('webgl2', params) as any;
      const isWebGL2 = !!glCtx;
      if (!isWebGL2) {
        glCtx = (canvasEl.getContext('webgl', params) || canvasEl.getContext('experimental-webgl', params)) as any;
      }
    ...
    function getSupportedFormat(glCtx: any, internalFormat: number, format: number, type: any): any {
    ...
    function supportRenderTextureFormat(glCtx: any, internalFormat: number, format: number, type: any) {
    ...
    function createFBO(w: number, h: number, internalFormat: number, format: number, type: any, param: any) {
    ...
    function createDoubleFBO(w: number, h: number, internalFormat: number, format: number, type: any, param: any) {
    ...
    function resizeFBO(target: any, w: number, h: number, internalFormat: number, format: number, type: any, param: any) {
    ...
    function resizeDoubleFBO(target: any, w: number, h: number, internalFormat: number, format: number, type: any, param: any) {
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      if (!firstMouseMoveHandled) {
        const colorVal = generateColor();
        updatePointerMoveData(pointer, posX, posY, colorVal);
  ...
  }, [mounted, hasMeasured, isMobile, SIM_RESOLUTION, DYE_RESOLUTION, CAPTURE_RESOLUTION, DENSITY_DISSIPATION, VELOCITY_DISSIPATION, PRESSURE, PRESSURE_ITERATIONS, CURL, SPLAT_RADIUS, SPLAT_FORCE, SHADING, COLOR_UPDATE_SPEED, BACK_COLOR, TRANSPARENT, RAINBOW_MODE, COLOR]);

  if (!mounted || !hasMeasured || isMobile) return null;
